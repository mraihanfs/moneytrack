...
def show(req):
    ...
    # fieldName = get_every_field(Transaction)
    dataJson = []
    for d in data:
...
def add_transaction(req):
    ...
    try:
        newTransaction = Transaction(description=description, transactionType=transactionType, valueCredit=value[0], valueDebet=value[1], sum_value=sum_value)
    # return HttpResponse(Transaction.objects.last().sum_value)
        newTransaction.save()
